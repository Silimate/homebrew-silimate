class Gtkx3 < Formula
  ...
  depends_on "libepoxy"
  depends_on "pango"
  # Add Broadway-specific dependencies
  depends_on "libsoup"  # Required for Broadway HTTP server
  depends_on "json-glib"  # Required for Broadway
  ...
  def install
    ...
    if OS.mac?
      ...
      # Enable Broadway backend with all necessary options
      args << "-Dbroadway_backend=true"
      args << "-Dwayland_backend=false"  # Disable Wayland to avoid conflicts
    end
    ...
    # Add Broadway-specific environment variables
    ENV["BROADWAY_BACKEND"] = "true"
    ENV["BROADWAY_BACKEND_ENABLED"] = "true"
